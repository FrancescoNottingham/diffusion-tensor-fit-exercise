...
def calculate_C_vals(data,s0_value,x,y,z):
    ...
    if s0[x,y,z] == 0:
        ...
        for i in np.arange(0,data.shape[-1]):
            C_vals[i] = -np.log(data[x,y,z,i]/s0_value)
        return C_vals
    
def calculate_diffusion_values(data,s0_value,bvals,bvecs,x,y,z):
    ...
    C = calculate_C_vals(data,s0_value,x,y,z)
        ...
        for y in np.arange(diffusion_data.shape[1]):
            for z in np.arange(diffusion_data.shape[2]):
                if brain_mask[x,y,z] == 0 or np.all(diffusion_data[x,y,z,:])==0:
                    ...
                    diffusion_tensor_values = calculate_diffusion_values(diffusion_data,s0_value,bvals,bvecs,x,y,z)
